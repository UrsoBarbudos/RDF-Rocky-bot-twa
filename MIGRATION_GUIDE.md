...
- [x] Убраны лишние endpoint'ы (оставлены только для Web Apps)
- [x] Добавлен webhook для уведомления n8n
- [x] **ОБНОВЛЕН** Worker для работы с таблицей "Пользователи" (`cloudflare-worker-users.js`)
- [x] **ОБНОВЛЕНЫ** HTML файлы registration.html и profile.html
- [x] **РАЗВЕРНУТ** обновленный Worker в Cloudflare
- [x] **ПРОТЕСТИРОВАНЫ** новые API endpoints
